...
JavaScript 스크립트는 `process` 객체를 통해 시스템 환경 변수에 접근할 수 있습니다:
...
셸 스크립트(Bash, PowerShell, AppleScript)는 시스템의 기본 환경 변수를 상속받아 실행됩니다.
...
- JavaScript 스크립트는 Node.js `vm` 컨텍스트에서 실행되지만, 전체 `require`, 전체 `process.env`, `Buffer`가 그대로 주입되어 사실상 제한 없는 Node.js 액세스를 가집니다. 실행 전 입력 정화, 명령 주입 필터링, 길이 제한, 형식 검증, 구문 검사는 수행되지 않습니다.
