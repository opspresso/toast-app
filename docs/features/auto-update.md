...
2. **수동 확인**: 사용자가 설정 창에서 "업데이트 확인" 버튼을 클릭할 때.

새 업데이트가 발견되면:

1. **설정 표시**: 설정 창에 업데이트 정보가 표시됩니다.
2. **앱 내 알림**: IPC를 통해 Toast 및 설정 윈도우에 업데이트 정보가 전송됩니다.
...
3. **업데이트 채널**: `channel: 'latest'` — 안정 채널 사용
...
1. **앱 시작**: 5초 후 자동 업데이트 확인 (프로덕션 모드에서만, silent 모드)
2. **업데이트 발견**: IPC를 통해 윈도우에 `update-available` 이벤트 전송
