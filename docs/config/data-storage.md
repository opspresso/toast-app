...
**위치** (Electron 의 `app.getPath('userData')` 는 `package.json` 의 `productName` 필드 `Toast` 를 사용):
...
> **참고**: `CONFIG_SUFFIX` 환경 변수가 설정되면 토큰 파일 이름이 `auth-tokens-${CONFIG_SUFFIX}.json` 으로 변경됩니다.

### 로그 파일

**위치** (`src/main/logger.js` 에서 `{userData}/logs/toast-app.log` 로 명시 설정):
