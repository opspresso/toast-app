...
- **드래그 앤 드롭**: 설정 모드에서 버튼을 드래그하여 위치 변경 가능
...
**목표**: 설정 모드에서 버튼을 드래그하여 원하는 위치로 재배열하기

1. **설정 모드 진입**
   - 전역 단축키를 눌러 Toast 창 열기
   - 콤마(,) 키를 누르거나 설정 아이콘(⚙️) 클릭하여 설정 모드 진입
   - 버튼들이 편집 가능한 상태로 변경됨

2. **드래그 앤 드롭 사용**
   - 이동하고 싶은 버튼 위에 마우스 커서 위치
   - 마우스 왼쪽 버튼을 누른 상태로 드래그 시작
   - 원하는 위치로 버튼을 드래그
   - 다른 버튼들이 자동으로 위치 조정됨
   - 원하는 위치에서 마우스 버튼 놓기

3. **변경 사항 저장**
   - 드래그 앤 드롭으로 변경된 버튼 순서가 즉시 적용됨
   - Escape 키를 누르거나 설정 아이콘을 다시 클릭하여 설정 모드 종료
   - 변경된 버튼 배치가 자동으로 저장됨
